...
class Hamming {
	...
	static int getParity(int b[], int p) {
		int parity = 0;
		for(int i=0 ; i < b.length ; i++) {
			if(b[i] != 2) {
				...
				//Now if the bit at the 2^(p) location of the binary value of index is 1
	...
	static void receive(int a[], int parity_count) {
		...
		int error_location = Integer.parseInt(cn, 2);
		if(error_location != 0) {
